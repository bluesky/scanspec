...
# To replace positional args in instantiations you can doit  like:
# Line\((.*), (.*), (.*), (.*)\) -> Line\(axis=$1, start=$2, stop=$3, num=$4\)
# Line\(([a-zA-Z_]*), ([0-9.]*), ([0-9.]*), ([0-9]*)\)
...
def test_zipped_lines() -> None:
    inst = Line(axis=x, start=0, stop=1, num=5).zip(
        Line(axis=y, start=1, stop=2, num=5)
    )
...
def test_zipped_product_lines() -> None:
    inst = Line(axis=y, start=1, stop=2, num=3) * Line(
        axis=x, start=0, stop=1, num=5
    ).zip(Line(axis=z, start=2, stop=3, num=5))
...
def test_rect_region() -> None:
    inst = Line(axis=y, start=1, stop=3, num=5) * Line(
        axis=x, start=0, stop=2, num=3
...
def test_rect_region_3D() -> None:
    inst = Static(axis=z, value=3.2, num=2) * Line(
        axis=y, start=1, stop=3, num=5
    ) * Line(axis=x, start=0, stop=2, num=3) & Rectangle(
        x_axis=x, y_axis=y, x_min=0, y_min=1, x_max=1.5, y_max=2.2
    )
...
def test_rect_region_difference() -> None:
    ...
    inst = (
        ...
        * Line(axis=x, start=0, stop=2, num=3).zip(Static(axis=DURATION, value=0.1))
        & Rectangle(x_axis=x, y_axis=y, x_min=0, y_min=1, x_max=1.5, y_max=2.2)
    ) - Rectangle(x_axis=x, y_axis=y, x_min=0.5, y_min=1.5, x_max=2, y_max=2.5)
...
def test_circle_region() -> None:
    inst = Line(axis=y, start=1, stop=3, num=3) * Line(
        axis=x, start=0, stop=2, num=3
...
def test_circle_snaked_region() -> None:
    inst = Mask(
        spec=Line(axis=y, start=1, stop=3, num=3)
...
def test_ellipse_region() -> None:
    inst = Line(axis="y", start=1, stop=3, num=3) * Line(
        ...
        x_axis=x, y_axis=y, x_middle=1, y_middle=2, x_radius=2, y_radius=1, angle=45
    )
    assert inst.axes() == [y, x]
    (dim,) = inst.calculate()
    assert dim.midpoints == {
...
def test_xyz_stack() -> None:
    ...
    spec = (
        Line(axis=z, start=0, stop=1, num=2)
        * ~Line(axis=y, start=0, stop=2, num=3)
        * ~Line(axis=x, start=0, stop=3, num=4)
    )
    dim = spec.frames()
    assert len(dim) == 24
    assert dim.lower == {
...
def test_multiple_statics():
    part_1 = (
        Static(axis="y", value=2)
        * Static(axis="z", value=3)
        * Line(axis="x", start=0, stop=10, num=2)
    )
    part_2 = (
        Static(axis="y", value=4)
        * Static(axis="z", value=5)
        * Line(axis="x", start=0, stop=10, num=2)
    )
...
def test_multiple_statics_with_grid():
    grid = Line(axis="y", start=0, stop=10, num=2) * Line(
        axis="x", start=0, stop=10, num=2
    )
