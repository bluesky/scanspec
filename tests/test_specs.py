...
def test_one_point_line() -> None:
    inst = Line(axis=x, start=0, stop=1, num=1)
    (dim,) = inst.calculate()
...
def test_two_point_line() -> None:
    inst = Line(axis=x, start=0, stop=1, num=2)
    (dim,) = inst.calculate()
...
def test_two_point_stepped_line() -> None:
    inst = step(Line(axis=x, start=0, stop=1, num=2), 0.1)
...
def test_two_point_fly_line() -> None:
    inst = fly(Line(axis=x, start=0, stop=1, num=2), 0.1)
    (dim,) = inst.calculate()
...
def test_one_point_bounded_line() -> None:
    ...
    assert inst == Line(axis=x, start=0.5, stop=1.5, num=1)


def test_many_point_bounded_line() -> None:
    inst = Line.bounded(x, 0, 1, 4)
    assert inst == Line(axis=x, start=0.125, stop=0.875, num=4)


def test_spiral() -> None:
    inst = Spiral(x_axis=x, y_axis=y, x_start=0, y_start=10, x_range=5, y_range=50, num=10)
    (dim,) = inst.calculate()
    assert dim.midpoints == {
        y: pytest.approx(
...
def test_spaced_spiral() -> None:
    ...
    assert inst == Spiral(x_axis=x, y_axis=y, x_start=0, y_start=10, x_range=10, y_range=10, num=78)


def test_zipped_lines() -> None:
    inst = Line(axis=x, start=0, stop=1, num=5).zip(Line(axis=y, start=1, stop=2, num=5))
...
def test_product_lines() -> None:
    inst = Line(axis=y, start=1, stop=2, num=3) * Line(axis=x, start=0, stop=1, num=2)
...
def test_zipped_product_lines() -> None:
    inst = Line(axis=y, start=1, stop=2, num=3) * Line(axis=x, start=0, stop=1, num=5).zip(
        Line(axis=z, start=2, stop=3, num=5))
...
def test_squashed_product() -> None:
    inst = Squash(spec=Line(axis=y, start=1, stop=2, num=3) * Line(axis=x, start=0, stop=1, num=2))
    assert inst.axes() == [y, x]
    (dim,) = inst.calculate()
    assert dim.midpoints == {
        x: pytest.approx([0, 1, 0, 1, 0, 1]),
...
def test_squashed_multiplied_snake_scan() -> None:
    inst = Line(axis=z, start=1, stop=2, num=2) * \
           Squash(spec=Line(axis=y, start=1, stop=2, num=2) *
...
def test_concat_lines() -> None:
    inst = Concat(left=Line(axis=x, start=0, stop=1, num=2), right=Line(axis=x, start=1, stop=2, num=3))
    assert inst.axes() == [x]
    (dim,) = inst.calculate()
...
def test_rect_region() -> None:
    inst = Line(axis=y, start=1, stop=3, num=5) * Line(axis=x, start=0, stop=2, num=3) & Rectangle(x_axis=x, y_axis=y,
                                                                                                   x_min=0, y_min=1,
                                                                                                   x_max=1.5, y_max=2.2)
    assert inst.axes() == [y, x]
    (dim,) = inst.calculate()
...
def test_rect_region_3D() -> None:
    inst = Static(axis=z, value=3.2, num=2) * \
           Line(axis=y, start=1, stop=3, num=5) * \
           Line(axis=x, start=0, stop=2, num=3) & \
...
def test_rect_region_intersection() -> None:
    inst = (
            Line(axis=y, start=1, stop=3, num=5) * Line(axis=x, start=0, stop=2, num=3)
            & Rectangle(x_axis=x, y_axis=y, x_min=0, y_min=1, x_max=1.5, y_max=2.2)
            & Rectangle(x_axis=x, y_axis=y, x_min=0.5, y_min=1.5, x_max=2, y_max=2.5)
    )
    assert inst.axes() == [y, x]
...
def test_rect_region_difference() -> None:
    ...
    inst = (
                   Line(axis=y, start=1, stop=3, num=5) * Line(axis=x, start=0, stop=2, num=3).zip(
               Static.duration(0.1))
                   & Rectangle(x_axis=x, y_axis=y, x_min=0, y_min=1, x_max=1.5, y_max=2.2)
           ) - Rectangle(x_axis=x, y_axis=y, x_min=0.5, y_min=1.5, x_max=2, y_max=2.5)
...
def test_circle_region() -> None:
    inst = Line(axis=y, start=1, stop=3, num=3) * \
           Line(axis=x, start=0, stop=2, num=3) & \
           Circle(x_axis=x, y_axis=y, x_middle=1, y_middle=2, radius=1)
    assert inst.axes() == [y, x]
    (dim,) = inst.calculate()
    assert dim.midpoints == {
...
def test_circle_snaked_region() -> None:
    inst = Mask(
        spec=Line(axis=y, start=1, stop=3, num=3) * ~Line(axis=x, start=0, stop=2, num=3),
        region=Circle(x_axis=x, y_axis=y, x_middle=1, y_middle=2, radius=1),
...
def test_ellipse_region() -> None:
    inst = Line(axis="y", start=1, stop=3, num=3) * \
           Line(axis="x", start=0, stop=2, num=3) & \
           Ellipse(x_axis=x, y_axis=y, x_middle=1, y_middle=2, x_radius=2, y_radius=1, angle=45)
    assert inst.axes() == [y, x]
    (dim,) = inst.calculate()
    assert dim.midpoints == {
...
def test_polygon_region() -> None:
    ...
    inst = Line(axis="y", start=1, stop=3, num=3) * \
           Line(axis="x", start=0, stop=4, num=5) & \
           Polygon(x_axis=x, y_axis=y, x_verts=x_verts, y_verts=y_verts)
...
def test_xyz_stack() -> None:
    ...
    spec = Line(axis=z, start=0, stop=1, num=2) * ~Line(axis=y, start=0, stop=2, num=3) * ~Line(axis=x, start=0, stop=3,
                                                                                                num=4)
    dim = spec.frames()
    assert len(dim) == 24
    assert dim.lower == {
...
def test_multiple_statics():
    part_1 = Static(axis="y", value=2) * Static(axis="z", value=3) * Line(axis="x", start=0, stop=10, num=2)
    part_2 = Static(axis="y", value=4) * Static(axis="z", value=5) * Line(axis="x", start=0, stop=10, num=2)
...
def test_multiple_statics_with_grid():
    grid = Line(axis="y", start=0, stop=10, num=2) * Line(axis="x", start=0, stop=10, num=2)
    part_1 = grid.zip(Static(axis="a", value=2)).zip(Static(axis="b", value=3))
    part_2 = grid.zip(Static(axis="a", value=4)).zip(Static(axis="b", value=5))
    spec = part_1.concat(part_2)
    assert list(spec.midpoints()) == [
        {"x": 0.0, "y": 0.0, "a": 2, "b": 3},
...
@pytest.mark.parametrize(
    "spec,expected_shape",
    [
        (Line(axis="x", start=0.0, stop=1.0, num=1), (1,)),
        (Line(axis="x", start=0.0, stop=1.0, num=5), (5,)),
        (Spiral(x_axis="x", y_axis="y", x_start=0.0, y_start=0.0, x_range=1.0, y_range=1.0, num=5, rotate=0.0), (5,)),
        (Line(axis="x", start=0.0, stop=1.0, num=2) * Line(axis="y", start=0.0, stop=1.0, num=2), (2, 2)),
        (Squash(spec=Line(axis="x", start=0.0, stop=1.0, num=2) * Line(axis="y", start=0.0, stop=1.0, num=2)), (4,)),
        (Zip(left=Line(axis="x", start=0.0, stop=1.0, num=2), right=Line(axis="y", start=0.0, stop=1.0, num=2)), (2,)),
        (Concat(left=Line(axis="x", start=0.0, stop=1.0, num=2), right=Line(axis="x", start=0.0, stop=1.0, num=2)),
         (4,)),
        (
                Line(axis="x", start=0.0, stop=1.0, num=2) * Line(axis="y", start=0.0, stop=1.0, num=2) * Line(axis="z",
                                                                                                               start=0.0,
                                                                                                               stop=2.0,
                                                                                                               num=2),
                (2, 2, 2),
        ...
        (
                Zip(left=Line(axis="x", start=0.0, stop=1.0, num=2), right=Line(axis="y", start=0.0, stop=1.0, num=2))
                * Line(axis="z", start=0.0, stop=2.0, num=2),
                (2, 2),
        ),
        (
                Concat(left=Line(axis="x", start=0.0, stop=1.0, num=2),
