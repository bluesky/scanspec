...
def test_one_point_line() -> None:
    ...
    assert dim.middle == {x: pytest.approx([0])}
...
def test_two_point_line() -> None:
    ...
    inst = Line(x, 0, 1, 2)
    (dim,) = inst.create_dimensions()
    assert dim.middle == {x: pytest.approx([0, 1])}
...
def test_two_point_stepped_line() -> None:
    ...
    assert dimx.middle == dimx.lower == dimx.upper == {x: pytest.approx([0, 1])}
    assert dimt.middle == dimt.lower == dimt.upper == {TIME: pytest.approx([0.1])}
...
def test_two_point_fly_line() -> None:
    ...
    (dim,) = inst.create_dimensions()
    assert dim.middle == {x: pytest.approx([0, 1]), TIME: pytest.approx([0.1, 0.1])}
...
def test_many_point_line() -> None:
    ...
    inst = Line(x, 0, 1, 5)
    (dim,) = inst.create_dimensions()
    assert dim.middle == {x: pytest.approx([0, 0.25, 0.5, 0.75, 1])}
...
def test_squashed_multiplied_snake_scan() -> None:
    ...
    for d in dimxyt.middle, dimxyt.lower, dimxyt.upper:
        assert d == {
            x: pytest.approx([4, 4, 6, 6, 6, 6, 4, 4]),
            y: pytest.approx([1, 1, 1, 1, 2, 2, 2, 2]),
            TIME: pytest.approx([9, 9, 9, 9, 9, 9, 9, 9]),
        }
    assert dimz.middle == dimz.lower == dimz.upper == {z: pytest.approx([1, 2])}
...
def test_concat_lines() -> None:
    ...
    assert inst.keys() == [x]
    (dim,) = inst.create_dimensions()
    assert dim.middle == {x: pytest.approx([0, 1, 1, 1.5, 2])}
...
def test_rect_region_3D() -> None:
    ...
    assert zdim.middle == {z: pytest.approx([3.2, 3.2])}
    assert zdim.middle is zdim.upper
    assert zdim.middle is zdim.lower
    assert xydim.middle == {
        x: pytest.approx([0, 1, 0, 1, 0, 1]),
        y: pytest.approx([1, 1, 1.5, 1.5, 2, 2]),
    }
