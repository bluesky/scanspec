...
def test_plot_1d_line_snake_repeat() -> None:
    runner = CliRunner()
    spec = 'Fly(2 * ~Linspace.bounded("x", 1, 2, 1))'
    with patch("scanspec.plot.plt.show"):
        result = runner.invoke(cli.cli, ["plot", spec])
    assert result.stdout == ""
    axes = plt.gcf().axes[0]
...
def test_plot_2d_line() -> None:
    runner = CliRunner()
    spec = 'Fly(Linspace("y", 2, 3, 2) * Snake(Linspace("x", 1, 2, 2)))'
    with patch("scanspec.plot.plt.show"):
        result = runner.invoke(cli.cli, ["plot", spec])
...
def test_plot_3d_line() -> None:
    runner = CliRunner()
    spec = (
        'Fly(Snake(Linspace("z", 5, 6, 2) '
        '* Linspace("y", 2, 3, 2) * Linspace("x", 1, 2, 2)))'
    )
    with patch("scanspec.plot.plt.show"):
        result = runner.invoke(cli.cli, ["plot", spec])
    assert result.exit_code == 0
